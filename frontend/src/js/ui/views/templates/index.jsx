						...
						<div className="flex justify-between">
							...
							<Form horizontal={true} className="w-40 form-no-margin">
								...
								<Switch
									label="Description"
									labelCol={9}
									value={state.importing.types.description}
									oninput={binder.checkbox(state.importing.types, 'description')}
								/>
								<Switch
									label="Print Template"
									labelCol={9}
									value={state.importing.types.printTemplate}
									oninput={binder.checkbox(state.importing.types, 'printTemplate')}
								/>
								<Switch
									label="List Template"
									labelCol={9}
									value={state.importing.types.listTemplate}
									oninput={binder.checkbox(state.importing.types, 'listTemplate')}
								/>
								<Switch
									label="Data Skeleton"
									labelCol={9}
									value={state.importing.types.skeletonData}
									oninput={binder.checkbox(state.importing.types, 'skeletonData')}
								/>
							</Form>
						</div>
						<div className="divider" />
							...
							<div className={`w-50 ${(i & 1) == 0 ? 'pr2' : ''}`}>
								<div className="flex ba b--black-10 h4 mb2 bg-white">
									<div className="flex-shrink-0 ph1 mr2 br b--black-05 bg-black-05">
										<Preview
											className="h-100"
											content={tryRender(t.printTemplate, tryParse(t.skeletonData))}
